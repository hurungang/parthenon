...
## Step 5: Update UX Prototype — ux_specialist Agent (if has_ui_changes)

If `.change.yaml` shows `has_ui_changes: true`:

Delegate to **ux_specialist agent**:
> "Read `docs/changes/<name>/prd.md` and `docs/changes/<name>/prototype/index.html`.
> Update the master product prototype at `docs/master/ux/prototype/index.html` to incorporate the new UI flows from this change.
> The master prototype mocks the FULL product — integrate the new flows without breaking existing ones.
...
> "Read `docs/changes/<name>/data-model.md` and `docs/config.yaml`.
> Apply the 'Master Data Model Update Instructions' section from the change doc to update `docs/master/data-model/`:
> 1. Update `docs/master/data-model/overview.md` ER diagram to include new/modified entities (Mermaid erDiagram)
> 2. Update `docs/master/data-model/modules/<module>/entities.md` with new entity names
> Data model must remain technology-agnostic — no schema code of any kind."
...
## Step 13: Update Change Status and Optionally Archive
...
If yes, move the change directory to the archive folder.
