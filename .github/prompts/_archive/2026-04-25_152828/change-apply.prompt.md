...
> "Read `docs/changes/<name>/data-model.md` and `docs/config.yaml` (for schema file paths). Update the project's schema/model files as described in the data-model.md. Follow the project's schema management convention — do NOT write migration scripts manually; use the project's schema tooling to generate them. Then update `docs/master/data-model/` with new entity information."
