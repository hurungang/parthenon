...
| `SessionExecutionLogsDialog` | Updated to fetch typed output when `output_id` is present on the session job; shows dual-tab layout (Execution Logs / Result) when a typed output is available; also shows Result tab for markdown/auto outputs when `output_data` is present on the job (FIX-20260703-110000); auto-switches to Result tab once output loads via `useEffect` + `hasAutoSwitchedRef` guard (prevents override if user manually switches back); uses `OutputTypeResultTab` for schema-aware rendering (typed path) and markdown rendering (markdown path); `useSessionExecutionLogStream` `onComplete` callback triggers re-fetch of session state to pick up `output_id` or `output_data` after stream ends. Located at `frontend/src/pages/agents/SessionExecutionLogsDialog.tsx`. |
...
| `WorkingStepsPanel` | component | **MODIFIED (FIX-20260701-120000)**: (1) Replaced 4s REST polling for sub-agent entries with `useSessionExecutionLogStream` SSE streaming when delegation is active — keeps initial REST fetch for historical entries and merges with live stream; (2) Added inline `output_type` chip and `result_preview` text display for `session_completed` step events | `frontend/src/components/executions/WorkingStepsPanel.tsx` |
| `AgentJobPage` | page | **MODIFIED (FIX-20260701-120000)**: Added `onViewSubAgentExecution` prop wired to `LogViewer`; adds `AgentExecutionDetailsDialog` sub-agent execution dialog triggered from delegation "View Execution Logs" button | `frontend/src/pages/agents/AgentJobPage.tsx` |
