...
There are no database dependencies, no frontend changes, and no modifications to existing Parthenon backend code. The only change to the existing codebase is a new service entry in `docker-compose.yml`.

---

## 2. Component Breakdown

### Demo App — Core Application
...
### New Endpoints — MCP Demo App

The demo app introduces two new HTTP endpoints. These are **not** part of the Parthenon backend; they are served by the standalone `mcp-demo-app` service.
...
|--------|---------------|----------|
| `initialize` | `protocolVersion`, `capabilities`, `clientInfo` | Server capabilities, protocol version, server info |
| `tools/list` | None | Array of tool descriptors from `TOOL_MANIFEST` |
| `tools/call` | `name: "helloWorld"`, `arguments: {}` | `{"message": "Hello from MCP Demo App!", "agent_sub": "<sub>", "agent_claims": {...}}` |
...
N/A — this change introduces no frontend components and no frontend state changes.
...
| Tool manifest | Static in-process constant (`TOOL_MANIFEST`) | No database; published to the Hub via the standard MCP `tools/list` response during tool sync |
| Agent identity claims | Request-scoped only | Extracted from the forwarded JWT on each tool call; not persisted |

All outbound HTTP calls use `httpx.AsyncClient` with async/await, consistent with the Parthenon backend pattern.
...
| `mcp_endpoint` | function | FastAPI route handler for `POST /mcp`; parses the JSON-RPC envelope and dispatches to `initialize`, `tools/list`, and `tools/call` handlers | `mcp-demo-app/app/routes/mcp.py` |
...
| `start.ps1` | script | PowerShell script to start the demo app locally; checks prerequisites, creates venv, installs deps, starts server on port 8001 | `mcp-demo-app/start.ps1` |
| `stop.ps1` | script | PowerShell script to stop the demo app; finds and kills process on port 8001 | `mcp-demo-app/stop.ps1` |
