# Agent Plan Mode — Epic PRD

## 1. Epic Overview

Today, users configuring new Agent Types lack a clear, actionable plan for how the agent will execute its work. This leads to uncertainty, misconfiguration, and inefficient agent deployment. The Agent Plan Mode feature will automatically generate and display an LLM-generated implementation plan for each agent when saved. The plan is created by an LLM analyzing the agent's instructions, associated SOPs, skills, and tools, producing a step-by-step execution strategy. This plan, along with a visual topology diagram, will give users confidence in agent setup, enable better preview and validation before execution, and reduce errors in agent operation. Critically, the saved plan becomes part of the agent's execution context — during runtime, the agent follows the pre-approved plan to ensure compliant, predictable behavior. By making agent logic transparent, reviewable, and enforcing plan-guided execution, this feature will accelerate onboarding, improve compliance, and drive more effective use of the agent platform.
...
- Users can preview the full plan and diagram before finalizing the agent save.
