...
- User-facing execution summaries derive conversational token usage and guardrail counters from the existing execution log stream.
- Agent Type guardrail configuration UI presents token budgets in k tokens and defaults the editor value to 1000k.
...
Frontend summary presentation
- Execution summary panels read the guardrail usage payload from execution logs and display current-session token usage for conversational runs.
- Guardrail editor views use a collapsed-by-default section that adapts its token-policy controls to the selected input type.
...
| LogPresenter | frontend/src/services/LogPresenter.ts | Derives execution-summary guardrail usage and conversational token visibility from log entries |
| LogSummaryPanel | frontend/src/components/logs/LogSummaryPanel.tsx | Renders guardrail usage chips and token-usage summary in the execution overview |
| AgentTypeForm | frontend/src/pages/agents/AgentTypeForm.tsx | Collapsed guardrail editor with input-type-aware token-policy controls and k-token budget display |
| AgentTypeDetailsDialog | frontend/src/components/agents/AgentTypeDetailsDialog.tsx | Read-only guardrail profile display with k-token budget formatting |
