...
- Surface conversational guardrail usage in the execution summary so operators can see live token usage alongside other guardrail signals.
...
- As an operator, I want execution summaries to show conversational guardrail usage so that I can confirm token consumption without opening raw logs.
...
- Control Center UI presents token budget values in k tokens with a default of 1000k, and the guardrail editor remains compact by default.
...
- Execution summaries expose conversational guardrail usage and guardrail outcomes in the user-facing session view.
