...
- Agent Runtime Monitor > shows trigger provenance on the tile and detail bubble
...
| 11 | Trigger provenance | Each node tile and its detail bubble surface the trigger source and source label (e.g. who or what triggered the agent). | e2e/tests/agent-runtime-monitor.spec.ts | Agent Runtime Monitor > shows trigger provenance on the tile and detail bubble |
