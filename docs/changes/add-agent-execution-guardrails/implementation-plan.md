...
- [ ] Guardrail policy contract is implemented and validated at Control Center boundaries
- [ ] Pre-execution cycle detection blocks recursive delegation before model execution starts
- [ ] Runtime enforces cumulative iteration, delegated depth/steps, and timeout limits
- [ ] Conversational sessions show current-session token consumption and allow continuation without token-budget-only hard stop
- [ ] Non-conversational and automated runs enforce token budget where available with deterministic fallback where unavailable
- [ ] Guardrail stop reasons propagate end-to-end across Agent Runtime, Communication Hub, and Control Center
- [ ] Execution log events include structured guardrail decision data for audit and triage
- [ ] Unit, integration, and A2A regression tests pass for all guardrail paths
- [ ] Documentation updates are complete and aligned with docs/config.yaml top-priority segregation rules
