...
- **Agent instructions for skills now automatically append a read-only Tool Section containing the schemas and usage descriptions of all selected tools. This Tool Section is always up-to-date and cannot be manually edited.**
- **Default skills for `save_result` and `send_notification` are created automatically during platform installation and are always present unless deleted by the user.**
...
- **Agent instruction field for skills now includes an auto-generated, read-only Tool Section based on tool selection**
- **Platform installation process now includes creation of default skills for `save_result` and `send_notification`**
...
- Update data model in `docs/master/data-model/modules/mcp-hub/entities.md` and `docs/master/data-model/modules/skills/entities.md` to include new/modified entities and relationships (sessions, tool-to-skill mapping, SOP instructions, default skills)
...
- **Document the automatic Tool Section in agent instructions and the default skills creation process**