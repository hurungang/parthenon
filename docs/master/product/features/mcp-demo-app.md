...
The MCP Demo App introduces a minimal Model Context Protocol (MCP) application that authenticates using the Keycloak ai_agent realm and exposes a single demo tool. Its purpose is to validate the end-to-end flow of agent identity within the MCP ecosystem, including passthrough session type, ensuring agents are recognized as first-class OIDC principals. This proof of concept enables seamless integration with the Parthenon MCP Hub and provides a reference for future agent-based capabilities, including passthrough session support.
...
- The app exposes a single demo tool, visible and callable after authentication
...
- Demo app supports passthrough session type, validating agent identity propagation without explicit session selection
- Documentation is provided to guide registration and integration steps, including passthrough session configuration
...
- No additional tools or business logic beyond the demo tool
