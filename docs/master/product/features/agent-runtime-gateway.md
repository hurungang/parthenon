...
# Agent Runtime with Gateway (Unified AI Agent Navigation)

## Epic Overview

The Agent Runtime with Gateway enables organizations to define agent types, manage agent roles and permissions, configure model access, and operate agents through a secure, observable gateway. With the unified "AI Agent" menu, all agent execution and monitoring features—including Agent Types, Agent Executions, and Agent Logs—are accessible from a single navigation structure. Agent identities are managed using the same identity provider as human users, but within a separate, configurable realm. The solution supports scalable, auditable, and policy-driven agent orchestration, ensuring compliance and operational transparency. The runtime leverages a modern agent framework for advanced skill-based execution and improved SOP/Skill support. Execution logs capture the full system instruction and user prompt for every agent run, providing complete visibility and traceability. All references to "Agent Instances" are now "Agent Executions."
...
- Provide visibility into agent executions, their statuses, and detailed execution history
- Ensure execution logs include the full system instruction and user prompt for every agent run, visible in the UI and accessible from both the Agent Executions dialog and Agent Logs module
- Leverage a modern agent framework to support skill-based execution and advanced SOP/Skill orchestration

## Users & Personas

- **Platform Administrators**: Configure agent types, roles, permissions, and model access; manage agent identities using OAuth sign-in to a dedicated realm; manage model configurations and credentials
- **Business Users**: Trigger agents to automate tasks, review results, and interact with agents as needed; view and filter agent executions and drill into execution details
- **Developers/Integrators**: Integrate agents into broader workflows via the gateway
- **Compliance & Audit Teams**: Require visibility into agent actions, permissions, session histories, and model usage
...
- As a platform admin, I want to assign agent identities to roles, so that I can control which identities are allowed to perform certain actions.
...
- As a platform admin, I want to refresh or re-authenticate agent identity tokens, so that agent access can be restored without requiring a new sign-in.
...
- As a platform admin or user, I want to view a dashboard of agent executions, filter by status, agent type, and time, so that I can monitor agent activity and performance.
...
- Agent runtime must use a modern agent framework (not LangGraph)
