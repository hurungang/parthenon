...
- Logs from `agent-runtime` contain `max_instances reached for agent_type_id=<id>`
...
2. Review the active instance count for the affected agent type in the Agent Runtime dashboard. Compare the count against `max_instances`. If the count is at or near the limit, determine whether the instances are legitimately active (users are waiting for responses) or whether they are stuck (no activity for an extended period).

3. Inspect Communication Hub logs for the active instance IDs. Look for instances that have no recent message activity and for which no `close` lifecycle event was received. A dropped network connection between the client and the Communication Hub can prevent the close event from reaching the runtime, leaving the instance counted as active indefinitely.
...
- The platform-wide default cap is set via `AGENT_RUNTIME_DEFAULT_MAX_INSTANCES` but can be overridden per agent type
