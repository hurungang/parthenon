...
- A scheduled job appears as triggered in Control Center (scheduling) logs but never reaches a completed state
...
1. Check the Control Center logs for the specific job's `trigger` event. Confirm the log shows the job was dispatched to the Agent Runtime with the correct agent type ID and prompt payload. If the trigger log entry is missing, the APScheduler job may have fired but failed to execute the dispatch function — check the Control Center for unhandled exceptions around the trigger time.

2. Check the Agent Runtime logs for the corresponding instance creation event. The log should contain an `instance created` entry with a reference back to the scheduled job's trigger. If this entry is missing, the dispatch call from the Control Center to the Agent Runtime failed — check network connectivity and confirm the Agent Runtime health endpoint is responding.

3. If the instance was created in the Agent Runtime but the job never completed, treat the agent instance as stuck and follow the agent instance limit runbook to identify and force-close the orphaned instance. Once the instance is closed, the job should be able to complete or be manually re-triggered.
...
- If multiple jobs are stuck simultaneously, the issue is more likely in the Agent Runtime (instance limit reached, service crash) than in the scheduling system — check the Agent Runtime health first
