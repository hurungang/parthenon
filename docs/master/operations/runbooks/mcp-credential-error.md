...
- Logs from the Control Center (MCP Hub component) contain `credential decryption failed` or `session not found`
...
4. If the credential is confirmed to be invalid or expired, re-enter the credentials for the affected session via the MCP Hub admin UI. The Control Center will encrypt the new credentials using the current `MCP_HUB_CREDENTIAL_ENCRYPTION_KEY` and store them, replacing the previous values.
